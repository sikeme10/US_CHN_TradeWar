...
p_ag <- ggplot(ag_map) +
  ...
  scale_fill_manual(values = ag_colors, name = "labor shares (ag)", na.value = "grey80", drop = FALSE) +
...
p_nonag <- ggplot(nonag_map) +
  ...
  geom_sf(color = "white", linewidth = 0.05) +
  geom_sf(fill = NA, color = "black", linewidth = 0.05)+ 
  scale_fill_manual(values = nonag_colors, name = "labor shares", na.value = "grey80", drop = FALSE) +
  facet_wrap(~subsector) +
  theme_void() +
  theme_map
