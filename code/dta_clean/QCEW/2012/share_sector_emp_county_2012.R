...
# Compute shares
sector_shares <- sector_emp %>%  left_join(total_emp, by = "fips") %>%
  mutate(share = emp_sector / emp_total) %>%
  select(fips, subsector, share) %>%
...
names(sector_shares)
write_csv(sector_shares,"/data/sikeme/TRADE/US_CHN_TradeWar_git/data/QCEW/subsector_share_county_2012.csv" )
...
# Compute total employment by CZ and sector
sector_emp <- dta %>%  group_by(fips, sector) %>%
  summarise(emp_sector = sum(emp, na.rm = TRUE), .groups = "drop")

# Compute total employment by CZ
total_emp <- dta %>%  group_by(fips) %>%
  summarise(emp_total = sum(emp, na.rm = TRUE), .groups = "drop")

# Compute shares
sector_shares <- sector_emp %>%  left_join(total_emp, by = "fips") %>%
  mutate(share = emp_sector / emp_total) %>%
  select(fips, sector, share) %>%
...
names(sector_shares)
write_csv(sector_shares,"/data/sikeme/TRADE/US_CHN_TradeWar_git/data/QCEW/sector_share_county_2012.csv" )
