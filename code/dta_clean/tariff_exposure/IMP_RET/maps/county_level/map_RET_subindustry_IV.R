################################################################################
# we create RET at County level (subsectors)
################################################################################
...
################################################################################
dta <- read_csv("/data/sikeme/TRADE/US_CHN_TradeWar_git/data/created_exposure/NAICS6/county/RET_elast_r_subsectors_IV.csv")
...
plot
ggsave(file.path(exp, "RET_tariff_2019_subsector.png"), plot, width = 8, height = 3.2, dpi = 300)

plot <- map_plot(county_map, "RET_tariff_bin", b_tariff, facet_yr = TRUE)
plot
ggsave(file.path(exp, "RET_tariff_subsector_year.png"), plot, width = 8, height = 5.5, dpi = 300)

################################################################################
# ── NTM ──────────────────────────────────────────────────────────────────────
plot <- map_plot(subset(county_map, year == 2019), "RET_NTM_bin", b_ntm)
plot
ggsave(file.path(exp, "RET_NTM_2019_subsector.png"), plot, width = 8, height = 3.2, dpi = 300)

plot <- map_plot(county_map, "RET_NTM_bin", b_ntm, facet_yr = TRUE)
plot
ggsave(file.path(exp, "RET_NTM_subsector_year.png"), plot, width = 8, height = 5.5, dpi = 300)

################################################################################
# ── NTM IV ───────────────────────────────────────────────────────────────────
plot <- map_plot(subset(county_map, year == 2019), "RET_NTM_IV_bin", b_ntm_iv)
plot
ggsave(file.path(exp, "RET_NTM_IV_2019_subsector.png"), plot, width = 8, height = 3.2, dpi = 300)

plot <- map_plot(county_map, "RET_NTM_IV_bin", b_ntm_iv, facet_yr = TRUE)
plot
ggsave(file.path(exp, "RET_NTM_IV_subsector_year.png"), plot, width = 8, height = 5.5, dpi = 300)
