...
################################################################################ 
dta <- read_csv("/data/sikeme/TRADE/US_CHN_TradeWar_git/data/created_exposure/NAICS6/RET_r_naics6_IV.csv")
...
table(cz_map$year)
names(cz_map)
cz_map <- cz_map %>% filter(!is.na(year)) %>% filter(year %in% c(2018:2019))
