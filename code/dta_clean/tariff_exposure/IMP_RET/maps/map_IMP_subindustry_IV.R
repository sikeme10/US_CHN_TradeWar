...
dta <- read_csv("/data/sikeme/TRADE/US_CHN_TradeWar_git/data/created_exposure/NAICS6/IMP_r_subsectors_IV.csv")
...
plot <- ggplot(subset(cz_map, year == 2019 )) +
  aes(fill = IMP_tariff_tot_ir_bin) +
  ...
  theme_map_autor
plot
ggsave(filename = file.path(exp, paste0("IMP_tariff_2019_subsector", "_autor.png")),
...
plot <- ggplot(subset(cz_map)) +
  ...
  theme_map
plot
ggsave(filename = file.path(exp, paste0("IMP_tariff_subsector.png")),
