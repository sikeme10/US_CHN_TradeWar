...
dta <- read_csv("/data/sikeme/TRADE/US_CHN_TradeWar_git/data/created_exposure/NAICS6/RET_r_industry_totlabor_IV.csv")
dta <- read_csv("/data/sikeme/TRADE/US_CHN_TradeWar_git/data/created_exposure/NAICS6/RET_r_industry_labor_IV.csv")
summary(dta)
names(dta)
...
unique(cz_map$year)
unique(cz_map$sector)
names(cz_map)

# ── Set sector filter ─────────────────────────────────────────────────────────
sector_selected <- "Ag"  # Options: "Ag", "Manu", "Other"
sector_selected <- "Manu"

sector_labels <- c("Ag" = "Agriculture", "Manu" = "Manufacturing", "Other" = "Other")
sec_label <- sector_labels[sector_selected]
...
cz_map <- cz_map %>%
  mutate(RET_tariff_bin = cut(RET_tariff_tot_r,
                              breaks = breaks,
                              labels = labels,
                              include.lowest = TRUE))

# ── Plot 2019 ─────────────────────────────────────────────────────────────────
plot <- ggplot(subset(cz_map, year == 2019 & sector == sector_selected)) +
  ...
  labs(title = paste("Retaliatory tariff exposure in 2019 -", sec_label)) +
  theme_void() +
  theme_map_autor

plot
ggsave(filename = file.path(exp, paste0("RET_tariff_2019_tot_", sector_selected, "_autor.png")),
...
plot <- ggplot(subset(cz_map, sector == sector_selected)) +
  aes(fill = RET_tariff_bin) +
  geom_sf(color = "white", linewidth = 0.05) +
  geom_sf(fill = NA, color = "black", linewidth = 0.2) +
  scale_fill_manual(values = colors, name = "RET Tariff", na.value = "grey80", drop = FALSE) +
  labs(title = paste("Retaliatory tariff exposure -", sec_label)) +
  theme_void() +
  facet_wrap(~year) +
  theme_map

plot
ggsave(filename = file.path(exp, paste0("RET_tariff_tot", sector_selected, ".png")),
...
plot <- ggplot(subset(cz_map, sector == sector_selected)) +
  ...
  labs(title = paste("Retaliatory NTM exposure -", sec_label)) +
  facet_wrap(~year) +
  theme_void() +
  theme_map

plot
ggsave(filename = file.path(exp, paste0("RET_NTM_tot_", sector_selected, ".png")),
       plot = plot, width = 5, height = 2, dpi = 300)
...
plot <- ggplot(subset(cz_map, sector == sector_selected)) +
  ...
  geom_sf(fill = NA, color = "black", linewidth = 0.2) +
  scale_fill_manual(values = colors, name = "RET NTM", na.value = "grey80", drop = FALSE) +
  labs(title = paste("Retaliatory NTM exposure (using IV) -", sec_label)) +
  facet_wrap(~year) +
  theme_void() +
  theme_map

plot
ggsave(filename = file.path(exp, paste0("RET_NTM_IV_tot_", sector_selected, ".png")),
       plot = plot, width = 5, height = 2, dpi = 300)
