...
colSums(is.na(merge_data1))
names(merge_data1)
test <- merge_data1 %>%  filter(is.na(ag_subsector))

# check subsector
unique(merge_data1$ag_subsector)
test <- merge_data1 %>%  filter(ag_subsector == "NA")
unique(test$subsector)
...
crop <- merge_data1 %>% filter(subsector== "crop")
names(crop)
table(crop$ag_subsector)
RET_crop_r <- crop %>% group_by(year, fips, ag_subsector) %>% 
...
summary(RET_crop_r)  
colSums(is.na(RET_crop_r))
write_csv(RET_crop_r, paste0("/data/sikeme/TRADE/US_CHN_TradeWar_git/data/created_exposure/NAICS6/county/RET_elast_r_crop_IV.csv") )



# b) aggregate at CZ level: get REP_r at livestock level
livestock  <- merge_data1 %>% filter(subsector== "livestock")
table(livestock$ag_subsector)
names(livestock)
RET_livestock_r <- livestock %>% group_by(year, fips, ag_subsector) %>% 
  summarise(RET_i_tariff = sum(RET_i_tariff, na.rm = TRUE),
            RET_i_NTB = sum(RET_i_NTB, na.rm = TRUE),
            RET_i_NTB_IV = sum(RET_i_NTB_IV, na.rm = TRUE),
...
colSums(is.na(RET_livestock_r))
write_csv(RET_livestock_r, paste0("/data/sikeme/TRADE/US_CHN_TradeWar_git/data/created_exposure/NAICS6/county/RET_elast_r_livestock_IV.csv") )
