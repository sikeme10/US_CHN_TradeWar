...
length(unique(industry_map$naics))
...
missing_in_HSNAICS <- setdiff(HS_NAICS$naics, industry_map$naics)
missing_in_HSNAICS <- setdiff(industry_map$naics,HS_NAICS$naics)
...
################################################################################
# 3. Aggregate at industry level from HS6
################################################################################
...
# Sum exports and output at industry evel
Merge <- Merge_trade_output %>% 
  group_by(year, ImporterISO3, industry ,industry_code) %>%
  summarise(
...
Merge_All <- Merge1 %>%  group_by(year, industry, industry_code) %>%
  summarise(
...
write_csv(Merge1, paste0(exp, "/gamma_iju_output_elast_IV.csv"))
write_csv(Merge_All, paste0(exp, "/RET_i_output_elast_IV.csv"))
