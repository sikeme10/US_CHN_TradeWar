...
exp <- "/data/sikeme/TRADE/US_CHN_TradeWar_git/output/FE/quaterly/"
...
dta <- read_csv("/data/sikeme/TRADE/US_CHN_TradeWar_git/data/created_gravity/dta_CHN_gravity_quaterly.csv")
names(dta)
colSums(is.na(dta))
...
for (i in seq_along(unique_HS2)) {
  ...
  sub_dta1 <- sub_dta %>%  group_by(ExporterISO3) %>%
    ...
    ungroup()
  sub_dta1 <- sub_dta1 %>%
    mutate(fe_id = interaction(quarter, year, ExporterISO3, hs4, drop = TRUE))
