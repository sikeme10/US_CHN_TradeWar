...
plot <- ggplot(subset(US, sector == "Ag"), aes(x = teJLMS, fill = factor(trade_war))) + 
  ...
  theme(panel.background = element_rect(fill = "white", color = NA),     
        plot.background  = element_rect(fill = "white", color = NA))
plot
ggsave(filename = file.path(exp, "plots_average/", "density_TE_US_Ag_2015_2020.png"),plot = plot, width = 8, height = 5, dpi = 300)
...
plot <- ggplot(subset(US, sector == "Ag"), aes(x = date, y = teJLMS)) +
  ...
  theme(panel.background = element_rect(fill = "white", color = NA),     
        plot.background  = element_rect(fill = "white", color = NA))
plot
ggsave(filename = file.path(exp, "plots_average/", "plot_TE_US_Ag_2015_2020.png"),plot = plot, width = 8, height = 5, dpi = 300)
...
plot <-ggplot(subset(US, sector == "Manu"), aes(x = teJLMS, fill = factor(trade_war))) + 
  ...
  theme(panel.background = element_rect(fill = "white", color = NA),     
        plot.background  = element_rect(fill = "white", color = NA))
plot
ggsave(filename = file.path(exp, "plots_average/", "density_TE_US_Manu_2015_2020.png"),plot = plot, width = 8, height = 5, dpi = 300)
...
plot <- ggplot(subset(US, sector == "Manu"), aes(x = date, y = teJLMS)) +
  ...
  theme(panel.background = element_rect(fill = "white", color = NA),     
    plot.background  = element_rect(fill = "white", color = NA))
plot
ggsave(filename = file.path(exp, "plots_average/", "plot_TE_US_Manu_2015_2020.png"),plot = plot, width = 8, height = 5, dpi = 300)
...
plot_te_over_time <- function(data, country_name, sector_name, exp_path) {
  ...
  dir.create(file.path(exp_path, "plots_average/Robust"), showWarnings = FALSE)
  
  # Build filename
  file_name <- paste0("plot_TE_", country_name, "_", sector_name, "_2015_2020.png")
  
  # Save the plot
  ggsave(  filename = file.path(exp_path, "plots_average/Robust", file_name),
