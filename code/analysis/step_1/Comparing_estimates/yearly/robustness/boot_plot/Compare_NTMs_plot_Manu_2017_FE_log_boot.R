...
length(unique(US_manu_2017_hs6$hs6_H5))
US_manu <-left_join(US_manu,US_manu_2017_hs6 )


# HS section level : weights for hs6 to aggregate to HS-section
US_manu_2017_hs_sect <- US_manu %>%  filter(year == 2017 & draw == 1) %>%
  ...
  ungroup()
US_manu <-left_join(US_manu,US_manu_2017_hs_sect )


# HS2 level : weights for hs6 to aggregate to Hs2
US_manu_2017_hs2 <- US_manu %>%  filter(year == 2017 & draw == 1) %>%
  group_by(hs2, hs6_H5) %>%
  summarise( Trade_value_USD_2017 = sum(Trade_value_USD, na.rm = TRUE),  .groups = "drop_last"    ) %>%
  group_by(hs2) %>%
  mutate(  hs2_tot_Trade_value_USD_2017 = sum(Trade_value_USD_2017),
           weight_hs2 = if_else( hs2_tot_Trade_value_USD_2017 > 0,   
                                 Trade_value_USD_2017 / hs2_tot_Trade_value_USD_2017,    NA_real_ )) %>%
  ungroup()
US_manu <-left_join(US_manu,US_manu_2017_hs2 )
...
plot <- ggplot(US_manu_q, aes(x = year)) +
  ...
  theme_trade
plot
ggsave(filename = file.path(exp, "plot/", "Compare_ln_AVE_manu_weight_2017_boot.png"),plot = plot, width = 8, height = 5, dpi = 300)
...
plot_FE <- ggplot(US_manu_FE_plot, aes(x = year)) +
  ...
  labs(title = "Weighted average Δ ln(1+AVE), Manufacturing (relative to 2017) — FE model",
    x = "Year",  y = "Weighted Δ ln(1+AVE)" ) +
  theme_trade
plot_FE
ggsave(filename = file.path(exp, "plot/", "Compare_ln_AVE_manu_weight_2017_boot.png"),plot = plot_FE, width = 8, height = 5, dpi = 300)
...
plot_FE_log <- ggplot(US_manu_FElog_plot, aes(x = year)) +
  ...
  scale_color_manual(values = c("FE_log" = "blue", "tariff" = "darkgreen"),
    name = "Variables"  ) +
  labs( title = "Weighted average Δ ln(1+AVE), Manufacturing (relative to 2017) — FE_log",
    x = "Year",    y = "Weighted Δ ln(1+AVE)"  ) +
  theme_trade

plot_FE_log
ggsave(filename = file.path(exp, "plot/", "Compare_ln_AVE_manu_weight_2017_boot_loglin.png"),plot = plot_FE_log, width = 8, height = 5, dpi = 300)
...
plot <- ggplot(US_manu_q_sect, aes(x = year)) +
  ...
  theme_trade
plot
ggsave(filename = file.path(exp, "plot/", "Compare_ln_AVE_manu_hs_sect_weight_2017_boot.png"),plot = plot, width = 9, height = 5, dpi = 300)
...
plot <- ggplot(US_manu_q_sect, aes(x = year)) +
  ...
  scale_color_manual(  values = c(    "FE_log"       = "blue",  "FE_log_mean"  = "orange",
                       ...
                       name = "Variables"  ) +
  facet_wrap(~ hs_section, scales = "free_y") +
  labs(    title = "Weighted average Δ ln(1+AVE), Manufacturing by HS section (log FE, relative to 2017)",
    x = "Year",    y = "Weighted Δ ln(1+AVE)"  ) +
  theme_trade
plot
ggsave(filename = file.path(exp, "plot/", "Compare_ln_AVE_manu_hs_sect_weight_loglin_2017_boot.png"),  plot = plot,
