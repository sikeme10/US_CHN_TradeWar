...
plot_FE <- ggplot(US_ag_FE_plot, aes(x = year)) +
  ...
  labs(title = "Weighted average Δ ln(1+AVE), Agriculture (relative to 2015) — FE model",
    x = "Year",  y = "Weighted Δ ln(1+AVE)" ) +
  theme_trade
plot_FE
...
plot_FE_log <- ggplot(US_ag_FElog_plot, aes(x = year)) +
  ...
  scale_color_manual(values = c("FE_log" = "blue", "tariff" = "darkgreen"),
    name = "Variables"  ) +
  labs( title = "Weighted average Δ ln(1+AVE), Agriculture (relative to 2015) — FE_log",
    x = "Year",    y = "Weighted Δ ln(1+AVE)"  ) +
  theme_trade
plot_FE_log
...
# Print them
plot_FE
ggsave(filename = file.path(exp, "plot/", "Compare_ln_AVE_Ag_weight_boot.png"),plot = plot_FE, width = 8, height = 5, dpi = 300)

plot_FE_log
ggsave(filename = file.path(exp, "plot/", "Compare_ln_AVE_Ag_weight_boot_loglin.png"),plot = plot_FE_log, width = 8, height = 5, dpi = 300)
