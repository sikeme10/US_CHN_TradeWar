...
# --- Test subset (uncomment to run on smaller D for debugging) ---
...
out_dir <- paste0(exp, "draw_batches_2017/")
...
for (b in seq_along(batches)) {
  ...
  # ---- 2017 baselines (per draw) ----
  pre17 <- dta[year == 2017,
               .(FE_pre_2017           = mean(FE,           na.rm = TRUE),
                 FE_pre_2017_bench     = mean(FE_bench,     na.rm = TRUE),
                 FE_pre_2017_wmean     = mean(FE_wmean,     na.rm = TRUE),
                 FE_log_pre_2017       = mean(FE_log,       na.rm = TRUE),
                 FE_log_pre_2017_bench = mean(FE_log_bench, na.rm = TRUE),
                 FE_log_pre_2017_wmean = mean(FE_log_wmean, na.rm = TRUE)),
               by = .(ExporterISO3, draw, hs4)]
  
  dta[pre17, `:=`(FE_pre_2017           = i.FE_pre_2017,
                  FE_pre_2017_bench     = i.FE_pre_2017_bench,
                  FE_pre_2017_wmean     = i.FE_pre_2017_wmean,
                  FE_log_pre_2017       = i.FE_log_pre_2017,
                  FE_log_pre_2017_bench = i.FE_log_pre_2017_bench,
                  FE_log_pre_2017_wmean = i.FE_log_pre_2017_wmean),
      on = c("ExporterISO3", "draw", "hs4")]
  rm(pre17); gc()
  
  # ---- Differences vs 2017 ----
  dta[, `:=`(
    diff_FE_2017 = fifelse(
      year > 2017 & !is.na(FE) & !is.na(FE_pre_2017),
      FE - FE_pre_2017, NA_real_),
    diff_FE_2017_bench = fifelse(
      year > 2017 & !is.na(FE) & !is.na(FE_pre_2017),
      (FE - FE_bench) - (FE_pre_2017 - FE_pre_2017_bench), NA_real_),
    diff_FE_2017_wmean = fifelse(
      year > 2017 & !is.na(FE) & !is.na(FE_pre_2017),
      (FE - FE_wmean) - (FE_pre_2017 - FE_pre_2017_wmean), NA_real_),
    diff_FE_log_2017 = fifelse(
      year > 2017 & !is.na(FE_log) & !is.na(FE_log_pre_2017),
      FE_log - FE_log_pre_2017, NA_real_),
    diff_FE_log_2017_bench = fifelse(
      year > 2017 & !is.na(FE_log) & !is.na(FE_log_pre_2017),
      (FE_log - FE_log_bench) - (FE_log_pre_2017 - FE_log_pre_2017_bench), NA_real_),
    diff_FE_log_2017_wmean = fifelse(
      year > 2017 & !is.na(FE_log) & !is.na(FE_log_pre_2017),
  ...
  dta[, `:=`(
    ...
    diff_ln_AVE_FE         = fifelse(year %in% 2018:2020, (1 / (1 - elasticities)) * diff_FE_2017,           NA_real_),
    diff_ln_AVE_FE_log     = fifelse(year %in% 2018:2020, (1 / (1 - elasticities)) * diff_FE_log_2017,       NA_real_),
    diff_ln_AVE_FE_bench   = fifelse(year %in% 2018:2020, (1 / (1 - elasticities)) * diff_FE_2017_bench,     NA_real_),
    diff_ln_AVE_FE_log_bench = fifelse(year %in% 2018:2020, (1 / (1 - elasticities)) * diff_FE_log_2017_bench, NA_real_),
    diff_ln_AVE_FE_wmean   = fifelse(year %in% 2018:2020, (1 / (1 - elasticities)) * diff_FE_2017_wmean,     NA_real_),
    diff_ln_AVE_FE_log_wmean = fifelse(year %in% 2018:2020, (1 / (1 - elasticities)) * diff_FE_log_2017_wmean, NA_real_),
    
    ln_AVE_FE_Chen              = (1 / (1 - CHEN_elasticities)) * FE,
    diff_ln_AVE_FE_Chen         = fifelse(year %in% 2018:2020, (1 / (1 - CHEN_elasticities)) * diff_FE_2017,           NA_real_),
    diff_ln_AVE_FE_log_Chen     = fifelse(year %in% 2018:2020, (1 / (1 - CHEN_elasticities)) * diff_FE_log_2017,       NA_real_),
    diff_ln_AVE_FE_bench_Chen   = fifelse(year %in% 2018:2020, (1 / (1 - CHEN_elasticities)) * diff_FE_2017_bench,     NA_real_),
    diff_ln_AVE_FE_log_bench_Chen = fifelse(year %in% 2018:2020, (1 / (1 - CHEN_elasticities)) * diff_FE_log_2017_bench, NA_real_),
    diff_ln_AVE_FE_wmean_Chen   = fifelse(year %in% 2018:2020, (1 / (1 - CHEN_elasticities)) * diff_FE_2017_wmean,     NA_real_),
    diff_ln_AVE_FE_log_wmean_Chen = fifelse(year %in% 2018:2020, (1 / (1 - CHEN_elasticities)) * diff_FE_log_2017_wmean, NA_real_)
  )]
...
merged_dir <- paste0(exp, "draw_batches_merged_2017/")
...
for (i in seq_along(batch_files)) {
  ...
  bf <- merge(bf, tariffs_hs4,
              ...
              all.x = TRUE)
  # NOTE: all.x = TRUE keeps every row from the batch. Tariff rows that don't
  # match a batch row would otherwise be added with NAs, inflating output.
  # Switch back to all = TRUE if you want full-join behavior.
...
################################################################################
# 7) Combine merged batches into final output (streaming)
################################################################################

final_path <- paste0(exp, "estimates_reduced_form_base_2017_FE_boot_elast.csv")
...
################################################################################
# 8) US subset — read each merged batch, keep USA rows only
################################################################################
...
fwrite(US, paste0(exp, "US_ln_NTMs_base_2017_FE_boot_hs4_elast.csv"))
rm(US); gc()