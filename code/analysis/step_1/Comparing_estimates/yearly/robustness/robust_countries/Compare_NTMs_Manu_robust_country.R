...
make_manu_lnAVE_plot <- function(Country, dta, save = FALSE, exp = NULL) {
  
  # 1. Filter country data and build date
  dta_country <- dta %>% filter(ExporterISO3 == Country)
  
  # 2. Manufacturing subset
  manu <- dta_country %>% 
    filter(sector == "Manu")
  ...
  manu <- left_join(manu, manu_2015_hs6, by = "hs6_H5")
  
  # 4. HS-section weights (HS6 -> HS-section)
  manu_2015_hs_sect <- manu %>%    filter(year == 2015) %>%
  ...
  manu <- left_join(manu, manu_2015_hs_sect, by = c("hs_section", "hs6_H5"))
  
  # 5. Weighted and simple averages over time
  manu_w <- manu %>%  filter(year %in% c(2016:2019)) %>%
    group_by(year) %>%
    summarise(
  ...
  plot <- ggplot(manu_w) +
    ...
    labs( title = paste0(Country, ": Weighted average Δ ln(1+AVE) for Manufacturing sector (relative to 2015)"),
  ...
  if (save) {
    if (is.null(exp)) {
      ...
      out_path <- file.path(exp, "plot/Robust/", paste0(Country, "_Compare_ln_AVE_manu_weight.png"))
...
unique(dta$ExporterISO3)
make_manu_lnAVE_plot("BRA", dta)
make_manu_lnAVE_plot("ARG", dta)
make_manu_lnAVE_plot("DEU", dta)
make_manu_lnAVE_plot("RUS", dta)
make_manu_lnAVE_plot("AUS", dta)
make_manu_lnAVE_plot("CAN", dta)
make_manu_lnAVE_plot("UKR", dta)

make_manu_lnAVE_plot("BRA", dta, save = TRUE, exp = exp)
make_manu_lnAVE_plot("ARG", dta, save = TRUE, exp = exp)
make_manu_lnAVE_plot("DEU", dta, save = TRUE, exp = exp)
make_manu_lnAVE_plot("RUS", dta, save = TRUE, exp = exp)
make_manu_lnAVE_plot("AUS", dta, save = TRUE, exp = exp)
make_manu_lnAVE_plot("CAN", dta, save = TRUE, exp = exp)
make_manu_lnAVE_plot("UKR", dta, save = TRUE, exp = exp)
...
make_manu_lnAVE_facet <- function(dta, countries = NULL, save = FALSE, exp = NULL) {
  ...
  manu <- dta %>%   filter(ExporterISO3 %in% countries,   sector == "Manu"    )
  ...
  if (save) {
    if (is.null(exp)) {
      ...
      out_path <- file.path(exp, "plot/Robust/", "AllCountries_Compare_ln_AVE_manu_weight.png")
...
make_manu_lnAVE_facet(dta, countries = countries_vec)
# To save:
make_manu_lnAVE_facet(dta, countries = countries_vec, save = TRUE, exp = exp)
...
plot_FE
ggsave(filename = file.path(exp, "/plot/", "change_ln_AVE_US_others_manu_FE.png"),
...
plot_FE_bench<- manu_w %>%
  ...
  labs(title = "Weighted average Δ ln(1+AVE) (FE model with benchmark) \n in the Manufacturing sector",
       x = "Year",  y = "Weighted Δ ln(1+AVE)" )
plot_FE_bench
ggsave(filename = file.path(exp, "/plot/", "change_ln_AVE_US_others_FE_manu_bench.png"),
       plot = plot_FE_bench, width = 10, height = 7, dpi = 300)

test <- manu_w %>% group_by(ExporterISO3) %>%
...
plot_tariff
ggsave(filename = file.path(exp, "/plot/", "change_ln_AVE_US_others_manu_tariff.png"),
...
plot_bemchmark <- FE_bench_hs4 %>%
  ...
  geom_line(linewidth = 0.5) +
  theme_trade +
  labs(title = "Benchmark FE value in the Manufacturing sector",
...
plot <- ggplot(US, aes(x = diff_ln_AVE_FE, y = diff_ln_AVE_u_tariff)) +
  ...
  theme_minimal() +
  labs(
    title = "Plot of FE and Tariff-adjusted Inefficiency (u) in Manufacturing Sector",
    x = "FE",
    y = "Tariff-adjusted u"  ) +
  theme(
