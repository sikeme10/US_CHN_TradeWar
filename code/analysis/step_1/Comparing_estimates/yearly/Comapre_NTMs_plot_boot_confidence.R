...
OUT_PLOT_DIR <- file.path(ROOT, "output/Compare_values/yearly/robust/boot_plot", as.character(BASE_YEAR))
...
p_sector <- ggplot(US_dta_q, aes(x = year)) +
  ...
  geom_ribbon(aes(ymin = FE_lo, ymax = FE_hi,  fill = "FE"), alpha = 0.2) +
  ...
  geom_ribbon(aes(ymin = FEb_lo, ymax = FEb_hi,  fill = "FE_bench"), alpha = 0.2) +
  ...
  scale_color_manual(values = legend_colors, breaks = legend_breaks, labels = legend_labels,
                     name   = "Variables" ) +
  scale_fill_manual(values = legend_fills,breaks = legend_breaks, labels = legend_labels,
                    name   = "Variables", guide  = "none")+
  labs( title = paste0("Weighted average Δ ln(1+AVE) in ", SECTOR_LABEL, " (relative to ", BASE_YEAR, ")"),
        x = "Year", y = "Weighted Δ ln(1+AVE)") +
  theme_trade
p_sector

# ggsave( filename = file.path(OUT_PLOT_DIR, paste0("Compare_ln_AVE_", SECTOR, "_sector_weight_boot", ".png")),        plot = p_sector, width = 8, height = 6, dpi = 300)
