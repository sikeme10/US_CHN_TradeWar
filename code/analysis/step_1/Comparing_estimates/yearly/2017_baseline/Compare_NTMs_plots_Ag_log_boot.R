...
US_ag_q <- US_ag_w %>%  group_by(year) %>%  summarise(
  # ---- FE (levels) ----
...
US_ag_q <- US_ag_q %>% mutate(chen_mean = if_else(year %in% c(2018:2019), chen_mean, NA))
US_ag_q <- US_ag_q %>% bind_rows(as_tibble( setNames(as.list(c(2017, rep(0, ncol(.) - 1))), names(.)))) %>%
  arrange(year)
...
plot
ggsave(filename = file.path(exp, "Compare_ln_AVE_Ag_weight_log_boot.png"),plot = plot, width = 8, height = 5, dpi = 300)
...
US_ag_q_sect <- US_ag_q_sect %>% mutate(chen_mean = if_else(year %in% c(2018:2019), chen_mean, NA))
# columns you want to set to 0 (everything except keys)
cols0 <- setdiff(names(US_ag_q_sect), c("year", "hs_section"))
add_2017 <- US_ag_q_sect %>%  distinct(hs_section) %>%  mutate(year = 2017) %>%
  mutate(!!!setNames(rep(list(0), length(cols0)), cols0))  # create cols as 0
US_ag_q_sect <- bind_rows(US_ag_q_sect, add_2017) %>%  arrange(hs_section, year)
...
plot <- ggplot(US_ag_q_sect, aes(x = year)) +
  ...
  theme_trade
plot
ggsave(filename = file.path(exp, "Compare_ln_AVE_Ag_hs_sect_weight_log_boot.png"),plot = plot, width = 9, height = 5, dpi = 300)
...
names(US_ag)
US_ag_w_hs2 <- US_ag %>%   group_by(year,draw, hs2) %>% 
...
# aggregate at hs section
US_ag_q_hs2 <- US_ag_w_hs2 %>%  group_by(hs2) %>%
  summarise( # FE (non-benchmark)
    FE_mean = mean(w_FE, na.rm = TRUE),
    FE_lo   = quantile(w_FE, 0.025, na.rm = TRUE),
    FE_hi   = quantile(w_FE, 0.975, na.rm = TRUE),
...
US_ag_q_hs2 <- US_ag_q_hs2 %>%  left_join(hs2_desc, by = "hs2")


library(writexl)
# write_xlsx( list("hs2_summary" = summary),  path = file.path(exp, "hs2_summary_ag.xlsx"))
