...
trade_dta <- dta %>% filter(draw == 1 & ExporterISO3 != "USA") %>% group_by(hs6_H5)
...
dta <- dta %>% group_by(year, draw, hs6_H5) %>%
  mutate(
    ...
    FE_mean = {
      m <- mean(FE[ExporterISO3 != "USA"], na.rm = TRUE)
      ifelse(is.nan(m), NA_real_, m)
    },
    FE_log_mean= {
      m <- mean(FE_log[ExporterISO3 != "USA"], na.rm = TRUE)
      ifelse(is.nan(m), NA_real_, m)
    }
  ) %>% 
  ungroup()
...
summary(dta$log_tariff)
test  <- dta %>% filter(is.na(log_tariff_pre_2015))
