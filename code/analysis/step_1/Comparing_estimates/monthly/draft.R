...
US_ag_w <- US_ag %>%
  filter(sector == "Ag", year > 2017) %>%  group_by(date) %>%
  summarise( 
...
plot <- ggplot(US_ag_w) +
  ...
  scale_color_manual( values = c(
    ...
    "sf_tariff" = "firebrick",
    "tariff"    = "darkgreen"    ),
    labels = c(  "FE" = "FE-based AVE",   "sf"        = "SF",   "sf_tariff" = "Tariff-adjusted SF",
                 "tariff"    = "Tariff"),
    name = "Series"  ) +
  labs(  title = "Weighted average Δ ln(1+AVE) for agricultural sector (relative to 2017 baseline) ",
         x = "Date",
...
plot <- ggplot(US_ag_w) +
  ...
  scale_color_manual(
    values = c(
      ...
      "sf_tariff" = "firebrick",
      "tariff"    = "darkgreen"    ),
    labels = c(  "FE"        = "FE-based AVE",
                 "sf"        = "SFA inefficiency (u)",
                 "sf_tariff" = "Tariff-adjusted inefficiency",
                 "tariff"    = "Tariff"    ),
    name = "Series"  ) +
  labs(  title = "Simple average Δ ln(1+AVE) for agricultural sector (relative to 2017 baseline) ",
         x = "Date",
...
names(US_ag)
US_ag_w_sect <- US_ag %>%
  filter(sector == "Ag", year > 2017) %>%  group_by(date, hs_section) %>%
  summarise( 
    w_FE        = weighted.mean(diff_ln_AVE_FE,        w = weight_section, na.rm = TRUE),
    w_sf        = weighted.mean(diff_ln_AVE_u,         w = weight_section, na.rm = TRUE),
    w_sf_tariff = weighted.mean(diff_ln_AVE_u_tariff,  w = weight_section, na.rm = TRUE),
    w_tariff    = weighted.mean(diff_log_tariff_2017,  w = weight_section, na.rm = TRUE),
    s_FE        = mean(diff_ln_AVE_FE,         na.rm = TRUE),
    s_sf        = mean(diff_ln_AVE_u,          na.rm = TRUE),
    s_sf_tariff = mean(diff_ln_AVE_u_tariff,   na.rm = TRUE),
    s_tariff    = mean(diff_log_tariff_2017,   na.rm = TRUE)  )
