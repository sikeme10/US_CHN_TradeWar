...
################################################################################
# create trade weights to aggregate at sector and HS section level
################################################################################
...
# Chen values are for 2018 and 2019, set the rest as NA 
US_ag  <- US_ag  %>% mutate(diff_log_tariff_Chen = if_else(year %in% c(2018,2019), diff_log_tariff_Chen, NA_real_ ),
                           diff_ln_AVE_chen     = if_else(year %in% c(2018,2019), diff_ln_AVE_chen,     NA_real_ )  )
...
US_ag_w <- US_ag %>%  filter(sector == "Ag", year > 2015) %>%  group_by(year, date) %>%
  summarise( 
  ...
  mutate( w_chen = if_else(year %in% c(2018,2019), mean(w_chen, na.rm=TRUE), NA),    
          s_chen = if_else(year %in% c(2018,2019), mean(s_chen, na.rm=TRUE), NA))


plot <- ggplot(subset(US_ag_w, year %in% c(2016,2019))) +
...
names(US_ag)
US_ag_w_sect <- US_ag %>% filter(sector == "Ag", year > 2015) %>%
...
plot <- ggplot(subset(US_ag_w_sect, year %in% c(2016,2019))) +
  geom_line(aes(x = date, y = w_FE,        color = "FE")) +
  geom_line(aes(x = date, y = w_sf,        color = "sf")) +
  geom_line(aes(x = date, y = w_sf_tariff, color = "sf_tariff")) +
  # geom_line(aes(x = date, y = w_chen,      color = "Chen_et_al")) +
