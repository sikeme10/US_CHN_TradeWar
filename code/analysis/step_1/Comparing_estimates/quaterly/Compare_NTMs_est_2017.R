...
trade <- trade |> 
  select(hs6_H5, year, quarter, ImporterISO3, ExporterISO3, 
         `HS6 Description`, hs6_H4, Trade_value_USD, Applied_tariff)

names(fe)
colSums(is.na(fe))

# Start from FE estimates as the base dataset
dta <- left_join(trade,fe)
colSums((is.na(fe)))
table(dta$year)
