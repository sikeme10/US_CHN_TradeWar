...
exp <- "/data/sikeme/TRADE/US_CHN_TradeWar_git/output/step_2/regression/schott/"
...
reg <- feols(EPOP ~ as.factor(year) |czone_2012 ,  data = dta)
summary(reg)

reg <- lm(employment ~ as.factor(year) + as.factor(czone_2012) ,   data = dta)
summary(reg)
...
reg <- lm(change_EPOP_2017 ~ IMP_tariff_r + RET_tariff_r + RET_NTB_r,
          data = sub_dta)
summary(reg)

reg <- lm(change_EPOP_2017 ~ IMP_tariff_r + RET_tariff_r + RET_NTB_r + SUB,
          data = sub_dta)
summary(reg)


reg <- lm(change_EPOP_2017 ~ IMP_tariff_r + RET_tariff_r + RET_NTB_r + as.factor(year),
          data = sub_dta)
summary(reg)


reg <- lm(change_EPOP_2017 ~ IMP_tariff_r + RET_tariff_r + RET_NTB_r + SUB + as.factor(year),
          data = sub_dta)
summary(reg)
...
reg_ntm <- feols(change_EPOP_2017_bis ~ as.factor(year)*IMP_tariff_r_2019 + as.factor(year)*RET_tariff_r_2019 +
                   as.factor(year)*RET_NTB_r_2019 + as.factor(year), data = sub_dta, weights = ~emp_2012)
