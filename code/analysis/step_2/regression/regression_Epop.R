...
reg <- feols(change_EPOP_2017_bis ~ IMP_tariff_r + RET_tariff_r + RET_NTB_IV_r+
                   SUB + share_ag_mining*year + year*share_mfg + share_other*year +
...
reg1 <- feols(change_EPOP_2017_bis ~ IMP_tariff_r + RET_tariff_r +
...
tbl <- etable(reg1, reg2,reg3,
              ...
              fitstat = ~ n + r2 + ar2 + f + f.p + rmse)
tbl
write_csv(as.data.frame(tbl), paste0(exp, "reg_main.csv"))
