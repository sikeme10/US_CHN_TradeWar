...
# 1) Tariffs only
reg_tar <- feols(change_EPOP_2017_bis ~ as.factor(year)*IMP_tariff_r_2019 + as.factor(year)*RET_tariff_r_2019 +
                   as.factor(year)*RET_NTB_r_2019 +     SUB +
                   share_forestry*year + share_crop*year + share_livestock*year +
                   share_nonag*year + share_other*year | year 
                 , data = sub_dta, weights = ~emp_2012, cluster = ~fips)
summary(reg_tar)

# 1) Tariffs only
reg_tar <- feols(change_EPOP_2017_bis ~ as.factor(year)*IMP_tariff_r_2019 + as.factor(year)*RET_tariff_r_2019 +
                   as.factor(year)*RET_NTB_IV_r_2019 +     SUB +
                   share_forestry*year + share_crop*year + share_livestock*year +
                   share_nonag*year + share_other*year | year 
                 , data = sub_dta, weights = ~emp_2012, cluster = ~fips)
summary(reg_tar)
...
# without employment weight
summary(sub_dta$change_EPOP_2017_bis)
reg1 <- feols(change_EPOP_2017_bis ~ IMP_tariff_r + RET_tariff_r + RET_NTB_r |
                year, data = sub_dta, cluster ~fips)
...
reg1 <- feols(change_EPOP_2017_bis ~   
                ...
                division^year + year, data = sub_dta,  cluster = ~fips)
